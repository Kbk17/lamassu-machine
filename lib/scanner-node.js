...
const capture = (device, { mode }, returnCallback, stillsCallback, process) => {
  if (!!activeStream) {
    console.log('[SCANNER] Camera is already open. Cleaning up and trying again...')
    try {
      // Próba czyszczenia obecnego strumienia
      activeStream.removeAllListeners()
      activeStream.destroy()
      activeStream = null
    } catch (err) {
      console.log('[SCANNER] Error cleaning up active stream:', err.message)
      return returnCallback(new Error('Camera open, could not clean up'), null)
    }
  }
