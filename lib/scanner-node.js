...
const capture = (device, { mode }, returnCallback, stillsCallback, process) => {
  if (!!activeStream) {
    console.log('[SCANNER] Camera is already open, ignoring new request')
    // Po prostu zwracamy bez błędu, aby nie zakłócać działania aplikacji
    return returnCallback(null, null)
  }
  ...
  try {
    ...
    console.log('[SCANNER] Error in camera capture:', err.message)
    cleanup()
    returnCallback(null, null)  // Zwracamy null zamiast błędu
  }
}
...
const delayedPhoto = (device, config, callback) => {
  if (!device) {
    console.log('[SCANNER] No camera device specified for delayed photo')
...
const delayedFacephoto = (callback) => {
  try {
    ...
    if (!device) {
      console.log('[SCANNER] No camera device configured for facephoto')
      return callback(new Error('No camera device configured for facephoto'), null)
    }

    console.log(`[SCANNER] Using device for facephoto: ${device}`)
    delayedPhoto(device, { mode }, (err, result) => {
      if (err) {
        console.log(`[SCANNER] Error in facephoto: ${err.message}`)
      }
      callback(err, result)
    })
  } catch (err) {
    console.log('[SCANNER] Exception in delayedFacephoto:', err.message)
    callback(err, null)
  }
}
