...
function cleanBarcode(rawBarcode) {
  ...
  // Nie logujemy danych z powodów bezpieczeństwa (kod parowania)
  ...
  if (cleaned.includes('ethereum:')) {
    ...
    if (match) {
      cleaned = match[1] + match[2]; // Tylko protokół + adres
    }
  }
  ...
  try {
    ...
    persistentScannerStream.on('data', (chunk) => {
      ...
      if (dataBuffer.includes('\n') || dataBuffer.includes('\r')) {
        ...
        // Nie logujemy kodu z powodów bezpieczeństwa (kod parowania)
...
function getBarcode() {
  ...
  currentScanPromise = new Promise((resolve, reject) => {
    ...
    currentCallback = (err, barcode) => {
      ...
      if (barcode) {
        // Sanityzuj dane przed zwróceniem
...
const scanMainQR = (cryptoCode, qrStillsCallback, resultCallback) => {
  ...
  getBarcode()
    .then((barcode) => {
      ...
      // Dodatkowa obsługa dla adresów Ethereum
