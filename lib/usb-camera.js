...
function takeFacePhoto(callback, options = {}) {
  if (options.cameraDebug) {
    console.log('!!! [USB-CAMERA] STARTING FACE PHOTO CAPTURE - ' + new Date().toISOString())
    console.log('!!! [USB-CAMERA] Options:', JSON.stringify(options))
    console.log('!!! [USB-CAMERA] Camera device:', cameraDevice || 'Not configured')
    console.log('!!! [USB-CAMERA] ffmpeg checked:', ffmpegChecked ? 'YES' : 'NO')
    console.log('!!! [USB-CAMERA] ffmpeg available:', ffmpegAvailable ? 'YES' : 'NO')
  } else {
    console.log('[USB-CAMERA] Starting face photo capture')
    console.log('[USB-CAMERA] Options:', JSON.stringify(options))
  }
...
function takeFacePhotoTC(callback) {
  console.log('!!! [USB-CAMERA] STARTING T&C PHOTO CAPTURE - ' + new Date().toISOString())
  console.log('!!! [USB-CAMERA] Camera device:', cameraDevice || 'Not configured')
  console.log('!!! [USB-CAMERA] ffmpeg available:', ffmpegAvailable ? 'YES' : 'NO')
  console.log('!!! [USB-CAMERA] isVerbose:', isVerbose ? 'YES' : 'NO')
  
  // Check if callback is a valid function
  if (typeof callback !== 'function') {
    console.log('!!! [USB-CAMERA] CRITICAL ERROR: callback is not a function!')
    return;
  }
  ...
  try {
    ...
    console.log('!!! [USB-CAMERA] Calling takeFacePhoto with options:', JSON.stringify(options))
    return takeFacePhoto((err, buffer) => {
      console.log('!!! [USB-CAMERA] CALLBACK FROM takeFacePhoto CALLED - ' + new Date().toISOString())
      console.log('!!! [USB-CAMERA] Error:', err ? 'YES: ' + err.message : 'NO')
      console.log('!!! [USB-CAMERA] Buffer:', buffer ? 'YES, size: ' + buffer.length + ' bytes' : 'NO')
      
      if (buffer) {
        // Try to save image to file for diagnostics
        const debugPath = '/tmp/tc-photo-debug-' + Date.now() + '.jpg';
        try {
          fs.writeFileSync(debugPath, buffer);
          console.log('!!! [USB-CAMERA] Saved photo copy to:', debugPath);
        } catch (saveErr) {
          console.log('!!! [USB-CAMERA] Failed to save photo copy:', saveErr.message);
        }
      }
    ...
    console.log('!!! [USB-CAMERA] CRITICAL ERROR in takeFacePhotoTC:', error.message)
...
function diagnosticPhotos() {
  console.log('[USB-CAMERA] Taking diagnostic photos...')
  ...
  return new Promise((resolve) => {
    ...
    // Check and log camera configuration
    console.log('[USB-CAMERA] Camera device used for diagnostics:', cameraDevice)
    console.log('[USB-CAMERA] ffmpeg status:', ffmpegAvailable ? 'Available' : 'Not available')
    
    // If camera is not available, return empty data
    if (!cameraDevice || !ffmpegAvailable) {
      console.log('[USB-CAMERA] No camera or ffmpeg, returning empty diagnostic data')
      return resolve(response)
    }
    
    // Take diagnostic photo and return it
    return takeDiagnosticPhoto({ saveDebug: true, debugPrefix: 'diagnostic-front' })
      .then(frontBuffer => {
        console.log('[USB-CAMERA] Diagnostic photo taken successfully, size:', frontBuffer.length, 'bytes')
        response.front = frontBuffer.toString('base64')
        console.log('[USB-CAMERA] Successfully converted photo to base64, size:', response.front.length, 'characters')
        return resolve(response)
      })
      .catch(err => {
        console.log('[USB-CAMERA] Error while taking diagnostic photo:', err.message)
        return resolve(response)
      })
  })
}
...
function capturePhotoWithFswebcam(options = {}) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      console.log('!!! [USB-CAMERA] Running command:', 'fswebcam', fswebcamArgs.join(' '))
      ...
      fswebcam.on('close', (code) => {
        ...
        if (code === 0) {
          ...
          try {
            ...
            console.log('!!! [USB-CAMERA] Read fswebcam image, size:', imageData.length, 'bytes')
            
            // Try to save a copy for diagnostics
            try {
            ...
            try {
              ...
              console.log('!!! [USB-CAMERA] Failed to remove temporary file:', unlinkErr.message)
            }
      ...
      fswebcam.on('error', (err) => {
        console.log('!!! [USB-CAMERA] Error launching fswebcam:', err.message)
        reject(err)
      })
    } catch (err) {
...
function takeFacePhotoTCAlternative(callback) {
  console.log('!!! [USB-CAMERA] RUNNING ALTERNATIVE METHOD FOR T&C PHOTO CAPTURE')
  
  // Check fswebcam availability
  exec('command -v fswebcam', (err, stdout) => {
    if (err) {
      console.log('!!! [USB-CAMERA] fswebcam not available, trying ffmpeg with file output')
      tryWithFfmpeg()
    } else {
      console.log('!!! [USB-CAMERA] fswebcam available:', stdout.trim())
      capturePhotoWithFswebcam()
        .then(buffer => {
          console.log('!!! [USB-CAMERA] fswebcam photo ready, passing to callback')
          callback(null, buffer)
        })
        .catch(error => {
          console.log('!!! [USB-CAMERA] fswebcam photo error, trying ffmpeg method:', error.message)
          tryWithFfmpeg()
        })
    }
  })
  ...
  function tryWithFfmpeg() {
    try {
      ...
      console.log('!!! [USB-CAMERA] Using ffmpeg with file output:', tmpFile)
      ...
      console.log('!!! [USB-CAMERA] Running ffmpeg command:', ffmpegArgs.join(' '))
      ...
      ffmpeg.on('close', (code) => {
        console.log('!!! [USB-CAMERA] ffmpeg finished with code:', code)
        
        if (code === 0) {
          try {
            const imageData = fs.readFileSync(tmpFile)
            console.log('!!! [USB-CAMERA] Read ffmpeg image, size:', imageData.length, 'bytes')
            
            // Save copy for diagnostics
            try {
              const debugPath = `/tmp/ffmpeg-tc-debug-${Date.now()}.jpg`
              fs.copyFileSync(tmpFile, debugPath)
              console.log('!!! [USB-CAMERA] Saved ffmpeg copy to:', debugPath)
            } catch (copyErr) {
              console.log('!!! [USB-CAMERA] Failed to save ffmpeg copy:', copyErr.message)
            }
            ...
            console.log('!!! [USB-CAMERA] Error reading ffmpeg file:', readErr.message)
            callback(readErr, null)
          }
        } else {
          console.log('!!! [USB-CAMERA] ffmpeg finished with error, stderr:', ffmpegOutput)
          callback(new Error(`ffmpeg finished with code ${code}`), null)
        }
      })
      
      ffmpeg.on('error', (err) => {
        console.log('!!! [USB-CAMERA] Error launching ffmpeg:', err.message)
        callback(err, null)
      })
    } catch (err) {
      console.log('!!! [USB-CAMERA] General ffmpeg error:', err.message)
      callback(err, null)
    }
  }
}
