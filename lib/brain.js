...
// At the beginning of the file, with other includes
...
const EXIT_TIME = 3600000  // 1 hour (3600000 ms)
...
const Brain = function (config) {
  ...
  this.isNewlandScanner = config.scanner && config.scanner.type === 'newland'
  this.shouldSaveQrAttempts = config.qrSnapshotDebug
  
  // Scanner initialization - using appropriate scanner based on configuration
  if (config.mockCam) {
    this.scanner = require('./mocks/scanner')
  } else if (this.isNewlandScanner) {
    this.scanner = require('./scanner-newland')
    console.log('Initialized Newland scanner for QR code scanning only')
  } else if (this.isGenmegaMachine) {
    this.scanner = require('./scanner-genmega')
  } else {
    this.scanner = require('./scanner-node')
  }

  this.scanner.config(config)

  // Initialize camera separately if using Newland scanner
  if (this.isNewlandScanner && config.frontFacingCamera && config.frontFacingCamera.device) {
    try {
      // Use scanner.js for camera functionality
      this.camera = require('./scanner')
      // Pass frontFacingCamera configuration as scanner device
      const cameraConfig = {
        ...config,
        scanner: {
          ...config.scanner,
          device: config.frontFacingCamera.device
        }
      }
      this.camera.config(cameraConfig)
      console.log('Initialized separate camera module for photo functions with device:', config.frontFacingCamera.device)
    } catch (err) {
      console.error('Failed to initialize camera module:', err.message)
    }
  }
  ...
  // Utwórz katalog na zdjęcia diagnostyczne
  try {
    const debugDir = '/tmp/lamassu-camera-debug';
    if (!fs.existsSync(debugDir)) {
      console.log(`[USB-CAMERA] Creating diagnostic directory: ${debugDir}`);
      fs.mkdirSync(debugDir, { recursive: true });
      console.log(`[USB-CAMERA] Diagnostic directory created: ${debugDir}`);
    }
  } catch (dirErr) {
    console.log('[USB-CAMERA] Error creating diagnostic directory:', dirErr.message);
  }
}
...
Brain.prototype.run = function run () {
  ...
  const callback = function () {
    ...
    // Just execute supervisorctl restart all
    cp.execFile('supervisorctl', ['restart', 'all'], {}, console.log)
  }

  // Initialize USB camera if front-facing camera is configured
  console.log('Initializing USB camera module')
  usbCamera.initialize(this.config)
    .then(cameraInitialized => {
      this.hasFrontFacingCamera = cameraInitialized
      if (cameraInitialized) {
        console.log('Front facing camera initialized successfully')
      } else {
        console.log('Front facing camera initialization failed')
      }
    })
    .catch(err => {
      console.log('Error initializing front facing camera:', err.message)
...
Brain.prototype._init = function init () {
  // Dodaj debugowanie
  console.log('=== BRAIN: Inicjalizacja komponentów ===')
  if (this.config && this.config.frontFacingCamera) {
    console.log('=== BRAIN: Konfiguracja kamery frontowej:', JSON.stringify(this.config.frontFacingCamera))
  } else {
    console.log('=== BRAIN: UWAGA: Brak konfiguracji kamery frontowej w obiekcie config!')
  }
...
Brain.prototype._initActionEvents = function _initActionEvents () {
  actionEmitter.on('brain', (action) => this.processBrainAction(action))
  actionEmitter.on('cryptoBuy', () => this.processPendingScreen())
  actionEmitter.on('cancelIdScan', () => this._cancelIdScan())
  actionEmitter.on('cancelScan', () => this._cancelScan())
  actionEmitter.on('scanBayLightOn', () => this.oldScanBayLightOn())
  actionEmitter.on('scanBayLightOff', () => this.oldScanBayLightOff())
  actionEmitter.on('startAddressScan', () => {
    console.log('BRAIN: Received startAddressScan event')
    if (_.includes(this.state, ['facephoto', 'faceConfirm', 'authorized', 'terms_accepted'])) {
      console.log('BRAIN: Current state is compatible, starting address scan')
      this._startAddressScan()
    } else {
      console.log('BRAIN: Cannot start address scan in current state:', this.state)
    }
  })
}
...
Brain.prototype.scanPDF = function scanPDF () {
  const callback = (err, result) => {
    if (err) return this._timedState('scanPDFFail')
    if (!result) return this._timedState('scanPDFFail')

    this.idCardData = result
    this._next()
  }

  this._transitionState('scanPDF')
  
  // Use separate camera module if available, otherwise use scanner
  const scannerToUse = this.isNewlandScanner && this.camera ? this.camera : this.scanner
  scannerToUse.scanPDF417(callback, _.noop)
}
...
Brain.prototype.scanPhotoCard = function scanPhotoCard () {
  const callback = (err, result) => {
    if (err) return this._timedState('scanPhotoFail')
    if (!result) return this._timedState('scanPhotoFail')

    this.idCardPhotoData = result
    this._next()
  }

  this._transitionState('scanPhotoCard')
  
  // Use separate camera module if available, otherwise use scanner
  const scannerToUse = this.isNewlandScanner && this.camera ? this.camera : this.scanner
  scannerToUse.scanPhotoCard(callback)
}
...
Brain.prototype.takeFacephoto = function takeFacephoto () {
  console.log('=== BRAIN: Starting face photo capture procedure ===')
  
  // Initialize photo capture procedure
  this._transitionState('facephoto')
  console.log('BRAIN: State set to facephoto')
  
  // Sprawdź stan bufora zdjęcia przed rozpoczęciem
  console.log('BRAIN: Current facephoto buffer state:', this.facephoto ? 'exists, size: ' + this.facephoto.length + ' bytes' : 'null')
  
  // Resetuj bufor zdjęcia przed rozpoczęciem nowego zdjęcia
  this.facephoto = null
  
  const callback = (err, buffer) => {
    if (err) {
      console.log('BRAIN: ERROR during face photo capture:', err.message)
      if (err.stack) console.log('BRAIN: Error stack:', err.stack)
      facephoto.dispatch('SCAN_ERROR')
      return this._timedState('facephotoFail')
    }
    
    if (!buffer || buffer.length < 1000) {
      console.log('BRAIN: No data or too small buffer from face photo capture:', buffer ? buffer.length : 0, 'bytes')
      facephoto.dispatch('SCAN_ERROR')
      return this._timedState('facephotoFail')
    }

    console.log('BRAIN: Face photo captured successfully, size:', buffer.length, 'bytes')
    
    // Zapisujemy zdjęcie w pamięci brain
    this.facephoto = Buffer.from(buffer)
    console.log('BRAIN: Photo saved to this.facephoto, size:', this.facephoto.length, 'bytes')
    
    // Zapisujemy również w module facephoto
    facephoto.setData(buffer)
    console.log('BRAIN: Photo saved to facephoto module, data available:', facephoto.getData() ? 'yes' : 'no')
    
    // Informujemy maszynę stanów, że zdjęcie zostało zrobione
    facephoto.dispatch('PHOTO_TAKEN')
    
    console.log('BRAIN: Photo process completed, proceeding to next state')
    this._next()
  }

  console.log('BRAIN: Starting face photo capture')
  
  // Check configuration
  console.log('BRAIN: Front camera configuration:', 
    this.config.frontFacingCamera ? JSON.stringify(this.config.frontFacingCamera) : 'none')
  console.log('BRAIN: hasFrontFacingCamera:', this.hasFrontFacingCamera)
  
  // Choose the best method for photo capture
  try {
    const usbCamera = require('./usb-camera')
    
    if (this.scanner && this.scanner.takeFacePhoto) {
      console.log('BRAIN: Using scanner module for face photo')
      this.scanner.takeFacePhoto(callback)
    } else if (this.hasFrontFacingCamera) {
      console.log('BRAIN: Using USB camera module for face photo')
      usbCamera.takeFacePhoto(callback)
    } else {
      console.log('BRAIN: No front camera available for face photo')
      callback(new Error('No front camera available'), null)
    }
  } catch (error) {
    console.log('BRAIN: CRITICAL ERROR during photo capture:', error.message)
    console.log('BRAIN: Error stack:', error.stack)
    callback(error, null)
  }
}

Brain.prototype.takeFacePhotoTC = function takeFacePhotoTC () {
  console.log('!!! [BRAIN] STARTING TO TAKE T&C PHOTO - ' + new Date().toISOString())
  
  // Save log to file
  try {
    const fs = require('fs')
    fs.appendFileSync('/tmp/brain-tc.log', 'Brain.takeFacePhotoTC called: ' + new Date().toISOString() + '\n')
  } catch (e) { 
    console.log('!!! [BRAIN] Error writing to log:', e.message)
  }
  
  // If usbCamera is missing, initialize it
  if (!this.usbCamera) {
    console.log('!!! [BRAIN] Initializing usbCamera module')
    this.usbCamera = require('./usb-camera')
  }
  
  // Try to initialize the camera if it failed
  if (!this.hasFrontFacingCamera) {
    console.log('!!! [BRAIN] Retrying front camera initialization')
    this.usbCamera.initialize(this.config)
      .then(initialized => {
        this.hasFrontFacingCamera = initialized
        console.log('!!! [BRAIN] Camera initialization result:', initialized ? 'success' : 'failure')
      })
      .catch(err => console.log('!!! [BRAIN] Error initializing camera:', err.message))
  }
  
  // Safely load facephoto module
  let facephoto;
  try {
    facephoto = require('./compliance/flows/facephoto');
    console.log('!!! [BRAIN] facephoto module loaded successfully');
  } catch (facephotoErr) {
    console.log('!!! [BRAIN] ERROR loading facephoto module:', facephotoErr.message);
  }
  
  const callback = (err, buffer) => {
    console.log('!!! [BRAIN] T&C PHOTO CALLBACK CALLED - ' + new Date().toISOString())
    console.log('!!! [BRAIN] Error in callback:', err ? 'YES: ' + err.message : 'NO')
    console.log('!!! [BRAIN] Buffer received:', buffer ? 'YES, size: ' + buffer.length + ' bytes' : 'NO')
    
    if (err) {
      console.log('!!! [BRAIN] ERROR taking photo for terms and conditions', err)
      // Continue even if there's an error, if we have a previously saved T&C photo
      if (facephoto && facephoto.getTCData()) {
        console.log('!!! [BRAIN] Found previously saved T&C photo, continuing')
        this._transitionState('terms_accepted')
        return
      }
    ...
    if (!buffer || buffer.length < 1000) {
      console.log('!!! [BRAIN] Buffer is empty or too small, size:', buffer ? buffer.length : 0)
      return // No state transition
    }
    
    console.log('!!! [BRAIN] Saving photo to customerTermsPhotoBuffer')
    this.customerTermsPhotoBuffer = Buffer.from(buffer) // Make a copy of the buffer
    
    // Save a copy in the facephoto module
    if (facephoto) {
      try {
        const savedBuffer = facephoto.setTCData(Buffer.from(buffer))
        console.log('!!! [BRAIN] Checking saved buffer in facephoto:', 
                    savedBuffer ? 'saved, size: ' + savedBuffer.length : 'not saved')
        
        // Check if the buffer was actually saved
        if (facephoto.getTCData()) {
          console.log('!!! [BRAIN] facephoto data saved correctly, getTCData() returns:', 
                      facephoto.getTCData().length, 'bytes')
        } else {
          console.log('!!! [BRAIN] ERROR: getTCData() does not return data after save')
        }
      } catch (facephotoErr) {
        console.log('!!! [BRAIN] Error saving to facephoto:', facephotoErr.message);
      }
    } else {
      console.log('!!! [BRAIN] facephoto module not available, cannot save photo');
    }
    
    console.log('!!! [BRAIN] Transitioning to terms_accepted state')
    // Wait a moment before changing state
    setTimeout(() => this._transitionState('terms_accepted'), 500)
  }
  
  try {
    console.log('!!! [BRAIN] Trying to take T&C photo')
    
    // First try our modified implementation
    // that will definitely take the photo and save it
    try {
      console.log('!!! [BRAIN] Trying to take photo directly through usbCamera')
      if (this.usbCamera) {
        this.usbCamera.takeFacePhoto(callback, { saveDebug: true, debugPrefix: 'tc-direct' })
        return
      }
    } catch (directErr) {
      console.log('!!! [BRAIN] Direct attempt failed:', directErr.message)
    }
    
    // Standard methods as fallback
    if (this.scanner && this.scanner.takeFacePhotoTC) {
      console.log('!!! [BRAIN] Using scanner module for T&C photo')
      this.scanner.takeFacePhotoTC(callback)
    } else if (this.usbCamera && this.usbCamera.takeFacePhotoTC) {
      console.log('!!! [BRAIN] Using USB camera module directly for T&C photo')
      this.usbCamera.takeFacePhotoTC(callback)
    } else {
      console.log('!!! [BRAIN] No available module to take T&C photo')
      callback(new Error('No available camera module'), null)
    }
  } catch (error) {
    console.log('!!! [BRAIN] CRITICAL ERROR during photo capture:', error.message)
    console.log('!!! [BRAIN] Error stack:', error.stack)
...
Brain.prototype.authorizeFacephotoData = function authorizeFacephotoData () {
  console.log('=== BRAIN: Starting face photo authorization ===')
  
  // Check photo data
  console.log('BRAIN: Available photo data:')
  console.log('BRAIN: - facephoto.getData():', facephoto.getData() ? 'exists, size: ' + facephoto.getData().length + ' bytes' : 'none')
  console.log('BRAIN: - this.facephoto:', this.facephoto ? 'exists, size: ' + this.facephoto.length + ' bytes' : 'none')
  ...
  return Promise.resolve()
    .then(() => {
      ...
      if (!data && !this.facephoto) {
        console.log('BRAIN: No face photo data available for authorization')
        return Promise.reject(new Error('No face photo data'))
      }
      
      // If no data in facephoto module, but exists in brain, use it
      const photoData = data || this.facephoto
      console.log('BRAIN: Authorizing face photo data, size:', photoData.length, 'bytes')
      
      // Save a copy of the photo for debugging
      try {
        const fs = require('fs')
        const debugPath = '/tmp/facephoto-debug-' + Date.now() + '.jpg'
        fs.writeFile(debugPath, photoData, (err) => {
          if (err) {
            console.log('BRAIN: Error saving debug photo:', err.message)
          } else {
            console.log('BRAIN: Debug photo saved to:', debugPath)
          }
        })
      } catch (debugErr) {
        console.log('BRAIN: Error in debug photo save:', debugErr.message)
      }
      
      return this.trader.updateCustomer(customer.id, {
        frontCameraData: photoData
      }, this.tx.id)
    })
    .then(result => {
      console.log('BRAIN: Face photo authorized successfully')
      this.customer = result.customer
      facephoto.dispatch('AUTHORIZED')
      
      // After successful authorization, we can proceed to scanning address
      console.log('BRAIN: Transitioning to scanning address')
      setTimeout(() => {
        console.log('BRAIN: Explicitly starting address scan after authorization')
        this._startAddressScan()
      }, 1000)
    ...
    .catch(err => {
      console.log('BRAIN: Face photo authorization error:', err.message)
      if (err.stack) console.log('BRAIN: Error stack:', err.stack)
      
      // In case of server error
      if (err.name === 'BadRequest' || err.statusCode) {
        this._fiatError(err)
      }
...
Brain.prototype.acceptTerms = function acceptTerms () {
  console.log('=== BRAIN: Starting terms acceptance (acceptTerms) ===')
  
  clearTimeout(this.screenTimeout)
  this.screenTimeout = null
  this.termsAcceptButtonPressed = true
  
  // Directly call the function to take the face photo before saving terms
  console.log('=== BRAIN: Taking face photo for T&C before saving acceptance ===')
  
  // Synchronously call the takeFacePhotoTC function
  this.takeFacePhotoTC()
  
  // Then continue with the terms acceptance process
  return this.trader.acceptTerms()
    .then(() => {
      console.log('=== BRAIN: Terms accepted, transitioning to idle state ===')
      this._transitionState('idle')
    })
    .catch(err => {
      console.log('=== BRAIN: Terms acceptance error:', err.message)
      this._transitionState('idle')
    })
}
...
Brain.prototype.failedQRScans = function failedQRScans (frames) {
  // Pomijamy zapisywanie nieudanych skanów dla skanera Newland
  if (this.isNewlandScanner) return Promise.resolve(null)
...
Brain.prototype._startPrintedWalletScan = function _startPrintedWalletScan () {
  ...
  console.log('Starting printed wallet scan')
  
  // Włączenie światła skanera tylko jeśli nie używamy skanera Newland
  if (!this.isNewlandScanner && this.hasNewScanBay()) {
    this.scanBayLightOn()
  }
  
  this.scanner.scanPK((err, pk) => {
    // Wyłączenie światła skanera tylko jeśli nie używamy skanera Newland
    if (!this.isNewlandScanner && this.hasNewScanBay()) {
    ...
    if (err) {
      console.log('Error scanning private key:', err.message)
      this.emit('error', err)
    }
    ...
    if (!freshState) {
      console.log('Not a fresh state, ignoring scan result')
      return
    }
    
    if (!pk) {
      console.log('No private key scanned')
      return this._idle()
    }
    
    if ((err && err.message === 'Invalid address') || this.pk !== pk) {
      console.log('Invalid private key or mismatch')
      return this._timedState('printerScanningError')
    }

    console.log('Successfully scanned private key')
    this.browser().send({ tx: this.tx })
    this._handleScan(this.tx.toAddress)
  })

  // Ustawiamy dłuższy timeout dla skanera Newland
  const timeoutDuration = this.isNewlandScanner ? QR_TIMEOUT * 2 : QR_TIMEOUT
  this.screenTimeout = setTimeout(() => {
    if (this.state !== 'printerScanAddress') return
    console.log('Screen timeout, cancelling scanner')
    this.scanner.cancel()
  }, timeoutDuration)
}

Brain.prototype.scanBayLightOn = function scanBayLightOn () {
  // Dla skanera Newland pomijamy włączanie światła
  if (this.isNewlandScanner) return
  
  emit('scanBayLightOn')
}

Brain.prototype.scanBayLightOff = function scanBayLightOff () {
  // Dla skanera Newland pomijamy wyłączanie światła
  if (this.isNewlandScanner) return
...
Brain.prototype.handleUnresponsiveCamera = function handleUnresponsiveCamera () {
  // For Newland scanner, we skip camera handling
  if (this.isNewlandScanner) {
    console.log('Using Newland scanner, skipping camera handling')
    return this._idle()
  }
...
Brain.prototype._cancelIdScan = function _cancelIdScan () {
  ...
  // Cancel scanner before transitioning to goodbye screen
  this.scanner.cancel()
  
  // For Newland scanner, we don't need camera handling
  if (this.isNewlandScanner) {
    return this._idle()
  }
...
Brain.prototype.hasNewScanBay = function hasNewScanBay () {
  // For Newland scanner, we always return false
  if (this.isNewlandScanner) return false
...
Brain.prototype._startAddressScan = function _startAddressScan () {
  ...
  console.log('Starting address scan with scanner type:', this.isNewlandScanner ? 'newland' : (this.isGenmegaMachine ? 'genmega' : 'standard'))

  // Enable scanner light only if we're not using Newland scanner
  if (!this.isNewlandScanner && this.hasNewScanBay()) {
    this.scanBayLightOn()
  }

  const frames = []
  const saveFailedScans = this.shouldSaveQrAttempts && !this.isNewlandScanner ?
    readFailedFrames(photo => frames.push(photo)) :
    null
    
  this.scanner.scanMainQR(this.tx.cryptoCode, saveFailedScans, (err, address) => {
    // Disable scanner light only if we're not using Newland scanner
    if (!this.isNewlandScanner && this.hasNewScanBay()) {
      this.scanBayLightOff()
    }
    ...
    if (err && err.message === 'Timeout') {
      console.log('Scanner timeout, returning to idle state')
      return this._idle()
    }
    ...
    if (err && err.message === 'Non-zero amount invoice supplied.') {
      console.log('Non-zero amount invoice supplied')
      return this._invalidAddress(true)
    }
    if (err) {
    ...
    if (!address) {
      console.log('Empty address from scanner')
      if (!this.isNewlandScanner) this.failedQRScans(frames)
      return this._idle()
    }
    
    console.log('Successfully scanned address:', address)
    this._handleScan(address)
  })

  // Set longer timeout for Newland scanner
  const timeoutDuration = this.isNewlandScanner ? QR_TIMEOUT * 2 : QR_TIMEOUT
  this.screenTimeout = setTimeout(() => {
...
Brain.prototype._tcEnabled = function _tcEnabled () {
  console.log('=== BRAIN: Checking if T&C screen should be displayed (_tcEnabled) ===')
  
  return this.trader.isTermsAccepted()
    .then(res => {
      if (res) {
        console.log('=== BRAIN: Terms already accepted, skipping ===')
        return false
      }
      
      console.log('=== BRAIN: Displaying T&C screen and setting timer ===')
      
      // Check if cameras are available
      if (this.scanner && this.scanner.takeFacePhotoTC) {
        console.log('=== BRAIN: Scanner with takeFacePhotoTC function is available ===')
      } else if (this.usbCamera) {
        console.log('=== BRAIN: USB Camera is available, takeFacePhotoTC:', 
                   this.usbCamera.takeFacePhotoTC ? 'YES' : 'NO')
      } else {
        console.log('=== BRAIN: WARNING: No available camera module for T&C ===')
      }
      
      this.screenTimeout = setTimeout(() => {
        console.log('=== BRAIN: T&C timer expired, automatically accepting terms ===')
        this.acceptTerms()
      }, this._getTermsDelayConfig().timeout)
...
Brain.prototype.updateTxCustomerPhoto = function updateTxCustomerPhoto ({ customerId, id, direction }) {
  // Get T&C photo data from facephoto module
  const tcPhotoData = facephoto.getTCData()
  
  // Get face photo data if available
  let facePhotoData = null
  if (this.facephoto) {
    console.log('Found face photo data in memory, size:', this.facephoto.length, 'bytes')
    facePhotoData = this.facephoto
    // After sending, clear data from memory
    this.facephoto = null
  } else {
    console.log('No face photo data in memory')
  }
  
  // If no photo data, we're done
  if (!tcPhotoData && !facePhotoData) {
    console.log('No photo data to send')
    return Promise.resolve()
  }
  
  console.log('Sending photo data to server:')
  console.log('- tcPhotoData:', tcPhotoData ? 'available, size: ' + tcPhotoData.length + ' bytes' : 'none')
  console.log('- facePhotoData:', facePhotoData ? 'available, size: ' + facePhotoData.length + ' bytes' : 'none')
  
  // Send photo data to server
  return this.trader.updateTxCustomerPhoto(id, customerId, { 
    tcPhotoData: tcPhotoData, 
    frontCameraData: facePhotoData,
    direction: direction 
  })
  .then(result => {
    console.log('Photos sent successfully')
    // Clear T&C photo data from facephoto module after successful send
    if (tcPhotoData) facephoto.cleanTCData()
    return result
  })
  .catch(err => {
    console.error('Error sending photos:', err.message)
    throw err
  })
}
...
Brain.prototype._diagnostics = function _diagnostics () {
  ...
  if (!this.hasFrontFacingCamera) {
    console.log('No front facing camera available for diagnostic photos')
    return this.trader.diagnosticPhotos({ scan: null, front: null })
      .then(() => console.log('Finished diagnostics'))
      .catch(err => console.log('Error running diagnostics:', err))
  }
  
  usbCamera.takeDiagnosticPhoto()
    .then(frontPhoto => {
      return this.trader.diagnosticPhotos({ 
        scan: null,
        front: frontPhoto
      })
    })
...
function testCameraCapture() {
  console.log('[USB-CAMERA] Testing photo capture...')
  
  return new Promise((resolve) => {
    if (!cameraDevice) {
      console.log('[USB-CAMERA] No configured camera device')
      return resolve(false)
    }
    
    // Create temporary file
    const tmpFile = `/tmp/camera-test-${Date.now()}.jpg`
    
    // Configure ffmpeg to save to file
    const args = [
      '-y',
      '-f', 'video4linux2',
      '-i', cameraDevice,
    ...
    console.log('[USB-CAMERA] Test command:', 'ffmpeg', args.join(' '))
    ...
    try {
      ...
      proc.stderr.on('data', (data) => {
        ...
        console.log('[USB-CAMERA] ffmpeg test stderr:', data.toString());
      });
      
      // Set timeout
      const testTimeout = setTimeout(() => {
        console.log('[USB-CAMERA] Photo capture test exceeded time limit')
        try {
          proc.kill('SIGKILL')
        } catch (e) {}
        resolve(false)
      ...
      proc.on('close', (code) => {
        ...
        if (code === 0 && fs.existsSync(tmpFile)) {
          try {
            ...
            console.log('[USB-CAMERA] Photo capture test successful. File:', tmpFile, 'Size:', fileStats.size, 'bytes');
            // Save as diagnostic as well
            const debugPath = '/tmp/lamassu-camera-debug/test-capture.jpg';
            try {
              // Make sure directory exists
              if (!fs.existsSync('/tmp/lamassu-camera-debug')) {
                fs.mkdirSync('/tmp/lamassu-camera-debug', { recursive: true });
              }
              fs.copyFileSync(tmpFile, debugPath);
              console.log('[USB-CAMERA] Diagnostic copy saved:', debugPath);
            } catch (copyErr) {
              console.log('[USB-CAMERA] Failed to save diagnostic copy:', copyErr.message);
            }
            
            resolve(true)
          } catch (statErr) {
            console.log('[USB-CAMERA] Error reading test file:', statErr.message);
            resolve(false)
          }
        } else {
          console.log('[USB-CAMERA] Photo capture test failed, code:', code, 'stderr:', stderrOutput);
          resolve(false)
        }
      ...
      proc.on('error', (err) => {
        clearTimeout(testTimeout)
        console.log('[USB-CAMERA] ffmpeg test process error:', err.message)
        resolve(false)
      })
    } catch (error) {
      console.log('[USB-CAMERA] Error starting photo capture test:', error.message)
      resolve(false)
    }
  })
}

function logProcessState(message) {
  try {
    const fs = require('fs')
    fs.appendFileSync('/tmp/brain-state.log', `${new Date().toISOString()} - ${message}\n`)
  } catch (e) {
    console.log('!!! [BRAIN] Error logging state:', e.message)
  }
}
