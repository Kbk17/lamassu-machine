...
Brain.prototype.distributeItems = function distributeItems(count, array) {
  ...
  return _.flow(
    arr => _.chunk(arr, Math.ceil(arr.length / count)),
...
Brain.prototype.scanPDF = function scanPDF () {
  ...
  this.scanner.scanPDF417((err, result) => {
    ...
    if (!this.isGenmegaMachine && (err || !result)) {
      ...
      this.trader.updateIdCardPhotos(customer.id, { photos: this.distributeItems(5, photos) })
        .then(
...
Brain.prototype.takeFacePhotoTC = function takeFacePhotoTC () {
  // Dodajemy blokadę, aby uniknąć wielu jednoczesnych wywołań
  if (this._tcPhotoInProgress) {
    console.log('[SCAN] TC photo already in progress, ignoring new request')
    return
  }
  
  this._tcPhotoInProgress = true
  
  try {
    // Sprawdź, czy kamera jest dostępna przed jej użyciem
    if (!this.hasFrontFacingCamera) {
      console.log('[SCAN] No front facing camera for T&C photo')
      this._tcPhotoInProgress = false
      return
    }
    
    console.log('[SCAN] Starting TC photo capture')
    this.scanner.delayedFacephoto((err, result) => {
      this._tcPhotoInProgress = false
      
      if (err) {
        // Ignorujemy błędy kamery, tylko logujemy
        console.log('[SCAN] Error taking T&C photo:', err.message || err)
        return
      }

      if (result) {
        try {
          console.log('[SCAN] TC photo captured successfully')
          facephoto.setTCData(result.toString('base64'))
        } catch (faceErr) {
          // Obsługa błędu ustawiania danych - nie krytyczna, tylko logujemy
          console.log('[SCAN] Error saving T&C face photo:', faceErr.message || faceErr)
        }
      }
    })
  } catch (e) {
    // Ogólna obsługa błędów - tylko logujemy, nie przerywamy flow
    console.log('[SCAN] Exception in takeFacePhotoTC:', e.message || e)
    this._tcPhotoInProgress = false
  }
}
...
Brain.prototype.startScreen = function startScreen () {
  const direction = this.tx && this.tx.direction

  // Sprawdź, czy kierunek transakcji jest zdefiniowany
  if (!direction) {
    console.log('[BRAIN] Transaction direction is undefined, returning to idle')
    return this._idle()
  }
  ...
  // Jeśli kierunek nie jest ani cashIn ani cashOut, logujemy błąd i wracamy do idle
  console.log(`[BRAIN] Invalid transaction direction: ${direction}`)
  return this._idle()
}
...
Brain.prototype._chooseCoin = function _chooseCoin (data) {
  // Jeśli przycisk jest już zablokowany, ignorujemy kolejne kliknięcia
  if (this.coinButtonLocked) {
    console.log('[BRAIN] Coin button already pressed, ignoring additional clicks')
    return
  }
  
  // Blokujemy przycisk natychmiast po pierwszym kliknięciu
  this.coinButtonLocked = true
  ...
  // Dodajemy krótkie opóźnienie przed przejściem do następnego ekranu
  // aby dać czas na dezaktywację przycisku w UI
  setTimeout(() => {
    this.startScreen()
    
    // Resetujemy blokadę przycisku po przejściu do innego ekranu
    // lub po pewnym czasie, aby przywrócić możliwość rozpoczęcia nowej transakcji
    setTimeout(() => {
      this.coinButtonLocked = false
    }, 1000)
  }, 100)
}
