...
Brain.prototype.run = function run () {
  ...
  // Sprawdzamy dostępność kamery niezależnie od typu skanera
  this.scanner.hasCamera('facephoto')
    ...
    .then(hasFrontFacingCamera => {
      ...
      if (hasFrontFacingCamera) {
        console.log('Front facing camera detected.')
      } else {
        console.log('Warning: no front facing camera detected.')
      }
    })
...
Brain.prototype.takeFacephoto = function takeFacephoto () {
  const callback = (err, buffer) => {
    if (err) return this._timedState('facephotoFail')
    if (!buffer) return this._timedState('facephotoFail')
...
Brain.prototype._processReal = function _processReal (req) {
  ...
  switch (req.button) {
    ...
    case 'pairingScanCancel':
      this.scanner.cancel()
      break
    case 'resetCameraLocks':
      // Nowa obsługa żądania resetowania blokad kamery
      if (this.scanner && this.scanner.resetCameraLocks) {
        console.log('Resetting camera locks from brain')
        this.scanner.resetCameraLocks()
          .then(() => console.log('Camera locks reset successfully'))
          .catch(err => console.log('Error resetting camera locks:', err))
      } else {
        console.log('Scanner module or resetCameraLocks method not available')
      }
      break
    case 'pairingErrorOk':
      this._unpaired()
...
Brain.prototype.startScreen = function startScreen () {
  // Check if this.tx exists and has a valid direction
  if (!this.tx || !this.tx.direction) {
    console.log('Attempting to transition to startScreen with invalid transaction state');
    return this._idle(); // Return to a safe state
  }
  ...
  // Additional safeguard if direction is neither cashIn nor cashOut
  console.log('Unknown transaction direction:', direction);
  return this._idle();
}
...
// Add a variable to track if terms acceptance is in progress
let acceptingTermsInProgress = false;

Brain.prototype.acceptTerms = function acceptTerms () {
  // Sprawdzamy globalny lock - jeśli jest aktywny, ignorujemy żądanie
  if (global.uiActionLock || this.termsAcceptButtonPressed) {
    console.log('UI action lock active or terms already accepted, ignoring request');
    return;
  }
  ...
  setTimeout(() => {
    ...
    // Zwolnienie locka po timeout (nie w tym samym miejscu co aktywacja)
    setTimeout(() => {
      global.uiActionLock = false;
      console.log('UI action lock released from acceptTerms');
    }, 1000);
  }, 1000);
}
