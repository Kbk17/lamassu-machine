...
Brain.prototype.takeFacePhotoTC = function takeFacePhotoTC () {
  console.log('!!! [BRAIN] STARTING T&C PHOTO CAPTURE - ' + new Date().toISOString())
  console.log('!!! [BRAIN] Scanner status:', this.scanner ? 'AVAILABLE' : 'NOT AVAILABLE')
  console.log('!!! [BRAIN] Front camera configuration:', 
    this.config.frontFacingCamera ? JSON.stringify(this.config.frontFacingCamera) : 'NONE')
  
  this._transitionState('face_photo_tc')
  console.log('!!! [BRAIN] State changed to face_photo_tc')
  ...
  const callback = (err, buffer) => {
    ...
    if (err) {
      ...
      console.log('!!! [BRAIN] Error stack trace:', err.stack || 'not available')
      this._transitionState('terms_accepted')
      return
    }
    
    if (!buffer || buffer.length < 1000) {
      console.log('!!! [BRAIN] Received invalid buffer (empty or too small)')
      this._transitionState('terms_accepted')
      return
    }
    
    console.log('!!! [BRAIN] Saving photo in customerTermsPhotoBuffer')
    this.customerTermsPhotoBuffer = buffer
    console.log('!!! [BRAIN] Moving to terms_accepted state')
    this._transitionState('terms_accepted')
  }
  
  try {
    console.log('!!! [BRAIN] Attempting to take photo...')
    
    // First try alternative method that writes directly to file
    const usbCamera = require('./usb-camera')
    
    console.log('!!! [BRAIN] Using alternative photo capture method')
    usbCamera.takeFacePhotoTCAlternative(function(alternativeErr, alternativeBuffer) {
      if (!alternativeErr && alternativeBuffer) {
        console.log('!!! [BRAIN] Alternative method succeeded, got photo!')
        return callback(null, alternativeBuffer)
      }
      
      console.log('!!! [BRAIN] Alternative method failed, trying standard approach')
      
      // If alternative method failed, try normal method
      if (this.scanner) {
        console.log('!!! [BRAIN] Using scanner for photo capture - scanner type:', 
          this.config.scanner ? this.config.scanner.type : 'unknown')
        this.scanner.takeFacePhotoTC(callback)
      } else {
        console.log('!!! [BRAIN] No scanner available, using usbCamera module directly')
        usbCamera.takeFacePhotoTC(callback)
      }
    }.bind(this))
    
    console.log('!!! [BRAIN] Photo function called, waiting for callback...')

  } catch (error) {
    console.log('!!! [BRAIN] CRITICAL ERROR during photo capture:', error.message)
    console.log('!!! [BRAIN] Error stack trace:', error.stack)
    callback(error, null)
  }
}
