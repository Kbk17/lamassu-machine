...
const COMPLIANCE_VERIFICATION_STATES = ['smsVerification', 'permission_id', 'permission_face_photo', 'usSsnPermission', 'customInfoRequestPermission', 'externalPermission', 'externalComplianceTimeout']
...
const EXTERNAL_COMPLIANCE_TIMEOUT = 120000
...
const REUSE_LAST_ADDRESS_TIMEOUT = 180000
...
const Brain = function (config) {
  ...
  this.cryptomatMaker = config?.cryptomatMaker ?? 'lamassu'
  this.cryptomatModel = this.cryptomatMaker === 'coincloud' ? 'coincloud' : (config?.cryptomatModel ?? 'sintra')

  this.shouldSaveQrAttempts = config.qrSnapshotDebug
  this.scanner = config.mockCam ? require('./mocks/scanner') :
    this.cryptomatMaker === 'genmega' ? require('./scanner-genmega') :
    require('./scanner-node')

  const liveviewEnabled = ['coincloud'].includes(this.cryptomatMaker)
  const scannerConfig = { liveviewEnabled, ...config }
  this.scanner.config(scannerConfig)
  this.liveviewEnabled = scannerConfig.liveviewEnabled // scanners can force-disable liveview
  ...
  const rejectedDenominations = (config.billValidator.rejectedDenominations || [])
  this.rejectedDenominationsMap = Object.fromEntries(rejectedDenominations.map((denom) => [denom, true]))
...
Brain.prototype.prunePending = function prunePending (txs) {
  const pendingTxs = txs.filter(tx => tx.dirty)
  ...
  const postTx = tx => {
    return this.postTx(modifier(tx))
      .catch(err => {
        if (err instanceof E.InvalidTransactionObject) return
  ...
  const promises = pendingTxs.map(postTx)
...
Brain.prototype.run = function run () {
  ...
  const self = this
  this._init()
  ledManager.run(this.cryptomatModel)
  ...
  const callback = function () {
    ...
    cp.execFile('supervisorctl', ['restart', 'all'], {}, function () {
      // We are not doing exit here because we want to restart the machine
      console.log('Restarting all supervisor services')
    })
  }
...
Brain.prototype._periodicLog = function _periodicLog () {
  ...
  const machine = this.cryptomatModel

  const tryAddTemperature = (clauses) => {
    const tempPromises = tempSensorPaths.map(path => readFile(path, { encoding: 'utf8' }))
  ...
  const tryAddDiskUsage = (clauses) => (
    new Promise((resolve, reject) => {
      const options = { encoding: 'utf8' }
      const callback = (err, stdout, stderr) => err ? reject(err) : resolve(stdout)
      return cp.execFile('df', ['--human-readable', '--local', '--output=pcent', '/'], options, callback)
    })
      .then((stdout) => {
        const pcent = stdout.split(os.EOL)[1].trim()
        clauses.push(`diskUsage: ${pcent}`)
      })
      .catch(() => {})
  )
  ...
  function reporting () {
    ...
    return Promise.allSettled([
      tryAddTemperature(clauses),
      tryAddDiskUsage(clauses),
    ])
      .finally(() => {
...
Brain.prototype.transitionScreen = function transitionScreen () {
  ...
  const customInfoRequest = appState === 'inputCustomInfoRequest'
    ? {
      customInfoRequest: _.get('customInfoRequest.customRequest')(_.find(trigger => trigger.customInfoRequest?.id === this.customInfoRequestId)(this.triggers))
    }
    : null
...
/**
 * Selects a specified number of items evenly distributed across the array
 * @param {number} count - Number of items to select
 * @param {Array} array - Source array
 * @returns {Array} - Evenly distributed selection
 */
Brain.prototype.distributeItems = function distributeItems(count, array) {
  if (array.length <= count) return array

  return _.flow(
    arr => _.chunk(Math.ceil(arr.length / count), arr),
    _.map(chunk => _.last(chunk)),
    arr => _.take(count, arr)
  )(array)
}
...
Brain.prototype.scanPDF = function scanPDF () {
  ...
  this.scanBayLightOn()
  const stillsCallback = photo => idCardData.addIdDataPhoto(photo)
  const resultCallback = (err, result) => {
    this.scanBayLightOff()
    this.startDisabled = false
    if (!this.cryptomatMaker === 'genmega' && (err || !result)) {
      const photos = this.distributeItems(5, idCardData.getIdDataPhotos())
...
Brain.prototype.triggerExternal = function triggerExternal (trigger) {
  ...
  return this.trader.getExternalCompliance(customer.id, trigger.id, this.externalComplianceRetry)
    .then(result => {
      this.externalComplianceURL = result.url
      // external.dispatch('URL', result.url)
      return this.showExternalCompliance(this.externalComplianceURL)
    })
    .catch(err => {
...
Brain.prototype.authorizePhotoCardData = function authorizePhotoCardData () {
  return Promise.resolve()
    .then(() => {
      ...
      const customer = this.customer
      const idCardPhotoData = idCardPhoto.getData()
      return this.trader.updateCustomer(customer.id, { idCardPhotoData }, this.tx.id)
    })
    .then(result => {
      this.customer = result.customer
...
Brain.prototype._connectedBrowser = function _connectedBrowser () {
  ...
  const cryptomatModel = this.cryptomatModel
...
Brain.prototype._processRequest = function _processRequest (req) {
  if (req.type === 'consoleLog') {
    return this._handleBrowserConsoleLog(req)
  }
...
Brain.prototype._handleBrowserConsoleLog = function _handleBrowserConsoleLog (log) {
  const prefix = `[BROWSER ${log.level.toUpperCase()}]`
  const formattedMessage = `${prefix}: ${log.message}`
...
Brain.prototype._processReal = function _processReal (req) {
  switch (req.button) {
    ...
    case 'retryPhotoScan':
      idCardPhoto.start({
        manual: (this.cryptomatMaker === 'lamassu' && this.cryptomatModel === 'sintra')
          || this.cryptomatMaker === 'coincloud'
      })
      break
    case 'fiatReceipt':
      this._fiatReceipt()
    ...
    case 'depositTimeoutNotSent':
      ...
      break
    case 'externalComplianceTimeoutYes':
      this.showExternalCompliance(this.externalComplianceURL)
      break
    case 'externalComplianceTimeoutNo':
      this.finishBeforeSms()
      break
    case 'cashOut':
      this._cashOut()
    ...
    case 'invalidAddressTryAgain':
      ...
      break
    case 'reuseLastAddress':
      this.reuseLastAddress()
      break
    case 'printAgain':
...
Brain.prototype._pairingScan = function _pairingScan () {
  this._timedState('pairingScan', { timeoutHandler: () => this.scanner.cancel() })
  this.browser().send({ action: 'pairingScan' })

  const resultCallback = (err, totem) => {
  ...
  this.scanner.scanQR({ resultCallback })
}
...
Brain.prototype.activate = function activate () {
  ...
  this.trader = require('./trader')(this.clientCert, connectionInfo, this.dataPath, this.cryptomatModel)
  ...
  const h = setInterval(() => {
    ...
    return this.billValidator.run(
      err => {
        ...
        this.billValidatorEnabled = true
...
Brain.prototype._idle = function _idle (locale) {
  ...
  this.externalComplianceURL = null
...
Brain.prototype.showExternalCompliance = function showExternalCompliance (url) {
  const timeoutHandler = () => this.finishBeforeSms()
  if (this.tx.direction === 'cashOut') {
    this._transitionState('externalCompliance', { externalComplianceUrl: url })
    return this._screenTimeout(() => this._timedState('externalComplianceTimeout', { timeoutHandler }), EXTERNAL_COMPLIANCE_TIMEOUT)
  }
...
Brain.prototype.authCompliance = function authCompliance (opts = {}) {
  ...
  flow.on('sendCode', data => {
    ...
    const promise = isEmailAuth ? this.trader.emailCode(dataField, this.tx.cryptoCode) : this.trader.phoneCode(dataField, this.tx.cryptoCode)
    promise.then(result => {
        this.customer = result.customer
        this.txBlockedByTrigger = false
        ...
        return flow.handle('requiredSecurityCode', result.code, this.trader.skip2fa)
      })
      .catch(err => {
...
Brain.prototype.requiredTiers = function requiredTiers (triggers, history, triggerTx) {
  ...
  const triggeredFormatted = _.map(o => o.customInfoRequest?.id ? _.assign(o, { requirement: o.customInfoRequest?.id }) : o, triggered)
...
Brain.prototype._start = function _start () {
  ...
  const lastUsedAddress = this.customer?.lastUsedAddress
  if (lastUsedAddress) {
    this.browser().send({
      lastUsedAddress,
      cryptoCode: this.tx.cryptoCode
    })
    return this._timedState('reuseLastAddress', {
      timeoutHandler: () => this._startAddressScan(),
      timeout: REUSE_LAST_ADDRESS_TIMEOUT
    })
  }
  this._startAddressScan()
}

Brain.prototype.reuseLastAddress = function reuseLastAddress () {
  this._handleScan(this.customer.lastUsedAddress)
}
...
Brain.prototype.failedQRScans = function failedQRScans (frames) {
  return this.shouldSaveQrAttempts ?
    this.trader.failedQRScans(this.distributeItems(5, frames)) :
...
Brain.prototype.hasNewScanBay = function hasNewScanBay () {
  return ['aveiro', 'sintra', 'gaia', 'tejo', 'grandola'].includes(this.cryptomatModel)
}
...
Brain.prototype._startAddressScan = function _startAddressScan () {
  ...
  const frames = []
  const stillsCallback = this.shouldSaveQrAttempts ?
    photo => frames.push(photo) :
    () => {}

  const resultCallback = (err, address) => {
    this.scanBayLightOff()
    clearTimeout(this.screenTimeout)
    this.startDisabled = false
...
Brain.prototype._transitionState = function _transitionState (state, auxData) {
  ...
  const rec = {
    action: state,
    direction: this.tx && this.tx.direction,
    // ensures liveview is enabled even if the browser is restarted
...
Brain.prototype.hasRejectedDenominations = function hasRejectedDenominations (bills) {
  return bills.some(({ denomination }) => this.rejectedDenominationsMap[denomination])
}
...
Brain.prototype._billsRead = function _billsRead (bills) {
  ...
  if (this.hasRejectedDenominations(bills)) {
...
Brain.prototype.runComplianceTiers = function (nonCompliantTiers) {
  ...
  if (this.cryptomatMaker === 'genmega' && tier === 'idCardPhoto') {
  ...
  complianceTiers.run(tier, this.cryptomatModel, _.find(['requirement', tier])(this.triggers))
}
...
Brain.prototype._billsRejected = function _billsRejected () {
  ...
  // if state is equal to returnState, transitionState doesn't emit a event
  // cash recyclers rely on the event to know when to reenable
...
Brain.prototype.verifyCustomerDiscounts = function verifyCustomerDiscounts (discount, source) {
  if (!_.isNil(discount)) {
    ...
    if (discount.discount > currentDiscount) {
      const couponId = source === 'promoCode' ? discount.id : null
      this.tx = Tx.update(this.tx, { discount: discount.discount, discountSource: source, couponId })
    }
  }
}
...
Brain.prototype._doSendCoins = function _doSendCoins () {
  ...
  if (this.state !== 'acceptingBills' && this.state !== 'acceptingRecyclerBills' && !_.includes(this.state, complianceStates)) {
    console.log('_doSendCoins: not in bills-accepting state')
    return
  }
...
Brain.prototype.ratesScreen = function () {
  ...
  this._timedState('rates', { data: { allRates: rates, ratesFiat: this.trader.locale.fiatCode }})
}
