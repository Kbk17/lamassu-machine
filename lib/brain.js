...
Brain.prototype.takeFacePhotoTC = function takeFacePhotoTC () {
  ...
  try {
    ...
    // Najpierw spróbujmy alternatywną metodę, która zapisuje bezpośrednio do pliku
    const usbCamera = require('./usb-camera')
    
    console.log('!!! [BRAIN] Używam alternatywnej metody wykonania zdjęcia')
    usbCamera.takeFacePhotoTCAlternative(function(alternativeErr, alternativeBuffer) {
      if (!alternativeErr && alternativeBuffer) {
        console.log('!!! [BRAIN] Alternatywna metoda powiodła się, mam zdjęcie!')
        return callback(null, alternativeBuffer)
      }
      
      console.log('!!! [BRAIN] Alternatywna metoda nie powiodła się, próbuję standardowe podejście')
      
      // Jeśli alternatywna metoda nie zadziałała, próbujemy normalnej metody
      if (this.scanner) {
