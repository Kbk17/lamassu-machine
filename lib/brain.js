...
const Brain = function (config) {
  ...
  this.shouldUseLiveView = ['coincloud'].includes(this.cryptomatMaker)
...
Brain.prototype.showExternalCompliance = function showExternalCompliance (url) {
  if (this.tx.direction === 'cashOut') {
    ...
    return this._screenTimeout(() => this._timedState('externalComplianceTimeout'), EXTERNAL_COMPLIANCE_TIMEOUT)
  }
...
Brain.prototype._makeLiveviewCallbacks = function _makeLiveviewCallbacks () {
  if (!this.shouldUseLiveView)
    return [null, () => {}]

  let enabled = false
  const liveviewEnabledCallback = (liveviewPort) => {
    this.browser().send({ action: 'enableLiveview', liveviewPort })
    enabled = true
  }

  const disableLiveview = () => {
    if (enabled)
      this.browser().send({ action: 'disableLiveview' })
  }

  return [liveviewEnabledCallback, disableLiveview]
}
...
Brain.prototype._startAddressScan = function _startAddressScan () {
  ...
  const [liveviewEnabledCallback, disableLiveview] = this._makeLiveviewCallbacks()
