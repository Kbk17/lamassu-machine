...
Brain.prototype.initialize = function initialize () {
  // Zapewnij, że mamy skonfigurowaną kamerę dla T&C
  if (!this.config.frontFacingCamera || !this.config.frontFacingCamera.device) {
    console.log('[BRAIN] UWAGA: Brak skonfigurowanej kamery frontowej, dodaję domyślną konfigurację /dev/video0')
    this.config.frontFacingCamera = { 
      device: '/dev/video0' 
    };
  }
  
  // Dodaj wymuszenie wykonywania zdjęcia dla T&C
  if (!this.config.terms) {
    console.log('[BRAIN] UWAGA: Brak aktywnych terms, włączam wykonywanie zdjęć T&C')
    this.config.terms = true;
  }
  
  console.log('[BRAIN] Konfiguracja kamery frontowej:', 
    this.config.frontFacingCamera ? JSON.stringify(this.config.frontFacingCamera) : 'BRAK')
  
  // Kontynuuj z oryginalną funkcją initialize
  this.clientCert = pairing.getCert(this.certPath)
...
Brain.prototype.takeFacephoto = function takeFacephoto () {
  ...
  const callback = (err, buffer) => {
    ...
    if (!buffer) {
      console.log('BRAIN: No data from face photo (empty buffer)')
      return this._timedState('facephotoFail')
    }

    console.log('BRAIN: Face photo captured successfully, size:', buffer.length, 'bytes')
    this.facephoto = buffer
    console.log('BRAIN: Photo saved in this.facephoto, moving to next state')
    this._next()
  }
...
Brain.prototype.takeFacePhotoTC = function takeFacePhotoTC () {
  console.log('!!! [BRAIN] ROZPOCZYNAM WYKONYWANIE ZDJĘCIA DLA T&C - ' + new Date().toISOString())
  console.log('!!! [BRAIN] Status skanera:', this.scanner ? 'DOSTĘPNY' : 'NIEDOSTĘPNY')
  console.log('!!! [BRAIN] Konfiguracja kamery frontowej:', 
    this.config.frontFacingCamera ? JSON.stringify(this.config.frontFacingCamera) : 'BRAK')
  
  this._transitionState('face_photo_tc')
  console.log('!!! [BRAIN] Stan zmieniony na face_photo_tc')
  
  const callback = (err, buffer) => {
    console.log('!!! [BRAIN] CALLBACK ZDJĘCIA T&C WYWOŁANY - ' + new Date().toISOString())
    console.log('!!! [BRAIN] Błąd w callback:', err ? 'TAK: ' + err.message : 'NIE')
    console.log('!!! [BRAIN] Buffer otrzymany:', buffer ? 'TAK, rozmiar: ' + buffer.length + ' bajtów' : 'NIE')
    
    if (err) {
      console.log('!!! [BRAIN] BŁĄD robienia zdjęcia dla terms and conditions', err)
      console.log('!!! [BRAIN] Stack trace błędu:', err.stack || 'niedostępny')
      this._transitionState('terms_accepted')
      return
    }
    
    if (!buffer || buffer.length < 1000) {
      console.log('!!! [BRAIN] Otrzymano nieprawidłowy bufor zdjęcia (pusty lub zbyt mały)')
      this._transitionState('terms_accepted')
      return
    }
    
    console.log('!!! [BRAIN] Zapisuję zdjęcie w customerTermsPhotoBuffer')
    this.customerTermsPhotoBuffer = buffer
    console.log('!!! [BRAIN] Przechodzę do stanu terms_accepted')
    this._transitionState('terms_accepted')
  }
  
  try {
    console.log('!!! [BRAIN] Próbuję wykonać zdjęcie...')
    
    // Zachowaj odniesienie do this w zmiennej self
    const self = this;
    const usbCamera = require('./usb-camera')
    
    // Najpierw spróbuj użyć standardowej metody z USB camera
    if (this.scanner) {
      console.log('!!! [BRAIN] Używam skanera do wykonania zdjęcia - typ skanera:', 
        this.config.scanner ? this.config.scanner.type : 'nieznany')
      this.scanner.takeFacePhotoTC(function(err, buffer) {
        if (!err && buffer && buffer.length > 1000) {
          console.log('!!! [BRAIN] Metoda skanera powiodła się!')
          callback(null, buffer);
        } else {
          console.log('!!! [BRAIN] Metoda skanera nie powiodła się, próbuję bezpośrednio usbCamera:', 
            err ? err.message : 'brak danych obrazu')
          
          // Próba bezpośredniego użycia usbCamera jako zapasowa metoda
          usbCamera.takeFacePhotoTC(callback);
        }
      });
    } else {
      console.log('!!! [BRAIN] Brak skanera, używam bezpośrednio modułu usbCamera')
      usbCamera.takeFacePhotoTC(callback)
    }
    
    console.log('!!! [BRAIN] Funkcja zdjęcia wywołana, czekam na callback...')
  } catch (error) {
    console.log('!!! [BRAIN] KRYTYCZNY BŁĄD podczas wywoływania funkcji robienia zdjęcia:', error.message)
    console.log('!!! [BRAIN] Stack trace błędu:', error.stack)
    callback(error, null)
  }
}
...
Brain.prototype.startScreen = function startScreen () {
  ...
  console.log('[BRAIN] startScreen: Sprawdzam terms, _tcEnabled:', this._tcEnabled(), 'termsAccepted:', this.tx.termsAccepted)
  
  // Sprawdź bezpośrednio, czy terms są aktywne i kamera jest dostępna
  if (this._tcEnabled() && !this.tx.termsAccepted) {
    console.log('[BRAIN] startScreen: Warunki T&C wymagane, sprawdzam kamerę')
    
    // Sprawdź dostępność kamery
    if (this.config.frontFacingCamera && this.config.frontFacingCamera.device) {
      try {
        const fs = require('fs');
        const cameraPath = this.config.frontFacingCamera.device;
        
        // Sprawdź czy urządzenie istnieje
        if (fs.existsSync(cameraPath)) {
          console.log('[BRAIN] startScreen: Urządzenie kamery istnieje:', cameraPath);
          this.hasFrontFacingCamera = true;
          
          // Wymuś wykonanie zdjęcia
          console.log('[BRAIN] startScreen: Wymuszam wykonanie zdjęcia T&C');
          this.takeFacePhotoTC();
        } else {
          console.log('[BRAIN] startScreen: Urządzenie kamery nie istnieje:', cameraPath);
          
          // Sprawdź /dev/video0 jako awaryjne
          if (fs.existsSync('/dev/video0')) {
            console.log('[BRAIN] startScreen: Znaleziono awaryjne urządzenie kamery: /dev/video0');
            this.config.frontFacingCamera.device = '/dev/video0';
            this.hasFrontFacingCamera = true;
        ...
        console.log('[BRAIN] startScreen: Błąd sprawdzania kamery:', err.message);
      }
    } else {
      // Jeśli brak konfiguracji kamery, spróbuj dodać domyślną
      console.log('[BRAIN] startScreen: Brak konfiguracji kamery, próbuję użyć /dev/video0');
      try {
        const fs = require('fs');
        if (fs.existsSync('/dev/video0')) {
          console.log('[BRAIN] startScreen: Znaleziono /dev/video0, dodaję do konfiguracji');
          this.config.frontFacingCamera = { device: '/dev/video0' };
          this.hasFrontFacingCamera = true;
        ...
        console.log('[BRAIN] startScreen: Błąd podczas sprawdzania /dev/video0:', err.message);
      }
    }
...
Brain.prototype.handleUnresponsiveCamera = function handleUnresponsiveCamera () {
  // Dla skanera Newland pomijamy obsługę kamery
...
Brain.prototype._cancelIdScan = function _cancelIdScan () {
  ...
  // Anulujemy skaner przed przejściem do ekranu pożegnalnego
  this.scanner.cancel()
  
  // Dla skanera Newland nie potrzebujemy obsługi kamery
  if (this.isNewlandScanner) {
    return this._idle()
  }
...
Brain.prototype.hasNewScanBay = function hasNewScanBay () {
  // Dla skanera Newland zawsze zwracamy false
...
Brain.prototype._tcEnabled = function _tcEnabled () {
  // Wymuszenie aktywacji terms dla celów testowych
  console.log('[BRAIN] _tcEnabled - Wymuszanie aktywacji terms dla testów');
  
  // Upewnij się, że trader.terms ma ustawiony active na true
  if (this.trader && this.trader.terms) {
    console.log('[BRAIN] _tcEnabled - Ustawiam trader.terms.active na true');
    this.trader.terms.active = true;
  } else if (this.trader) {
    console.log('[BRAIN] _tcEnabled - Inicjalizuję trader.terms');
    this.trader.terms = { active: true };
  }
  
  // Zaloguj faktyczny stan
  const originalValue = this.trader && _.get(['active'], this.trader.terms);
  console.log('[BRAIN] _tcEnabled - Oryginalny stan terms:', originalValue, 'wymuszam: true');
  
  // Zawsze zwracaj true dla testów
  return true;
}

Brain.prototype._tcPhotoEnabled = function _tcPhotoEnabled () {
  const termsEnabled = !!this.config.terms
  const hasCameraConfig = !!(this.config.frontFacingCamera && this.config.frontFacingCamera.device)
  
  // Jeśli kamera jest skonfigurowana, spróbujmy zweryfikować jej dostępność
  if (hasCameraConfig && !this.hasFrontFacingCamera) {
    // Spróbujmy wykonać szybki test dostępności kamery
    const fs = require('fs');
    try {
      const cameraPath = this.config.frontFacingCamera.device;
      if (fs.existsSync(cameraPath)) {
        console.log('[BRAIN] Kamera znaleziona pod ścieżką:', cameraPath);
        // Kamera istnieje fizycznie, więc możemy ją uznać za dostępną
        this.hasFrontFacingCamera = true;
      }
    } catch (e) {
      console.log('[BRAIN] Błąd sprawdzania urządzenia kamery:', e.message);
    }
  }
  
  const cameraActive = !!this.hasFrontFacingCamera
  // Dodatkowy parametr, który może być używany do wymuszenia działania kamery
  const forceCameraEnabled = !!this.config.forceCameraEnabled
  
  console.log('[BRAIN] _tcPhotoEnabled - terms:', termsEnabled, 'camera config:', hasCameraConfig, 'active:', cameraActive, 'forced:', forceCameraEnabled)
  
  // Kamera jest aktywna, jeśli jest faktycznie aktywna LUB wymusiliśmy jej włączenie
  const effectiveCameraActive = cameraActive || forceCameraEnabled
  
  return termsEnabled && hasCameraConfig && effectiveCameraActive
}
