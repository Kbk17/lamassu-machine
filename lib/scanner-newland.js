...
function takeFacePhotoTC(callback) {
  console.log('[NEWLAND] Delegowanie wykonania zdjęcia twarzy TC do modułu usb-camera')
  
  try {
    // Użyj dedykowanego modułu usb-camera
    const usbCamera = require('./usb-camera')
    
    // Upewnij się, że moduł jest zainicjalizowany z konfiguracją
    if (configuration) {
      usbCamera.initialize(configuration)
    }
    
    // Wywołaj funkcję z modułu usb-camera
    usbCamera.takeFacePhotoTC(callback)
  } catch (error) {
    console.log('[NEWLAND] Błąd podczas używania usb-camera dla takeFacePhotoTC:', error.message)
    callback(error, null)
  }
}
