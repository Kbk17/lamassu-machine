...
global.isCameraBeingUsed = false; // Nowy, prosty flaga - znacznie bardziej niezawodna
...
function cancel() {
  ...
  if (activeStream) {
    ...
    try {
      ...
      // Zwolnij flagę używania kamery
      global.isCameraBeingUsed = false;
      console.log('[NEWLAND] Camera usage flag released after cancel');
    } catch (err) {
...
function isOpened() {
  return isRunning || !!activeStream || global.isCameraBeingUsed;
}
...
const capture = (device, { mode }, returnCallback, stillsCallback, process) => {
  ...
  // Ustawienie flagi używania kamery
  global.isCameraBeingUsed = true;
  console.log('[NEWLAND] Camera usage flag activated');

  const externallyClosedHandler = () => {
    // Zwalnianie flagi w przypadku zewnętrznego zamknięcia
    global.isCameraBeingUsed = false;
    console.log('[NEWLAND] Camera closed externally, flag released');
  ...
  const cleanup = () => {
    ...
    activeStream.destroy();
    activeStream = null;
    // Zwalnianie flagi przy czyszczeniu
    global.isCameraBeingUsed = false;
    console.log('[NEWLAND] Camera cleanup complete, flag released');
  }
  ...
  try {
    ...
    // Zwalnianie flagi w przypadku błędu
    global.isCameraBeingUsed = false;
    console.log('[NEWLAND] Camera error occurred, flag released');
    returnCallback(err, null);
  }
}
