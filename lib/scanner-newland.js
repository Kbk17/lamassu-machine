...
global.lastCameraOperation = 0;
global.lastCancelTime = 0;
global.isCameraBeingUsed = false; // Flaga podstawowa dla używania kamery
global.cameraInitiationInProgress = false; // Nowa flaga - blokowanie inicjalizacji podczas trwającego procesu
const CAMERA_OP_COOLDOWN = 5000; // 5 sekund między operacjami kamery
const CANCEL_COOLDOWN = 6000; // 6 sekund cooldown po anulowaniu
...
// Nowa funkcja do resetowania lockówi flag kamery przy nowej transakcji
function resetCameraLocks() {
  console.log('[NEWLAND] Resetting all camera locks and flags for new transaction');
  global.uiActionLock = false;
  global.lastCameraOperation = 0;
  global.lastCancelTime = 0;
  global.isCameraBeingUsed = false;
  global.cameraInitiationInProgress = false;
  
  // Upewnij się, że wszystkie zasoby kamery są zwolnione
  if (activeStream) {
    try {
      console.log('[NEWLAND] Cleaning up leftover camera stream during lock reset');
      activeStream.destroy();
      activeStream = null;
    } catch (err) {
      console.log('[NEWLAND] Error destroying camera stream during reset:', err.message);
    }
  }
...
function cancel() {
  ...
  // Zapisz czas anulowania
  global.lastCancelTime = Date.now();
  console.log('[NEWLAND] Setting last cancel time:', global.lastCancelTime);
  ...
  if (activeStream) {
    ...
    try {
      activeStream.destroy();
      // Zwolnij flagi używania kamery
      global.isCameraBeingUsed = false;
      global.cameraInitiationInProgress = false;
      console.log('[NEWLAND] Camera flags released after cancel');
    } catch (err) {
      console.log('[NEWLAND] Error destroying camera stream:', err.message);
    }
    activeStream = null;
  }
...
function isOpened() {
  return isRunning || !!activeStream || global.isCameraBeingUsed || global.cameraInitiationInProgress;
}
...
const capture = (device, { mode }, returnCallback, stillsCallback, process) => {
  // Sprawdź, czy inicjalizacja kamery jest już w trakcie
  if (global.cameraInitiationInProgress) {
    console.log('[NEWLAND] Camera initiation already in progress. Blocking duplicate request.');
    return returnCallback(new Error('Camera initiation in progress'), null);
  }
  
  // Prosta, bardzo skuteczna blokada wielokrotnego otwarcia kamery
  if (global.isCameraBeingUsed) {
    console.log('[NEWLAND] Camera is already in use. Blocking duplicate request.');
    return returnCallback(new Error('Camera already in use'), null);
  }

  if (!!activeStream) {
    console.log('[NEWLAND] Camera is already open. Shouldn\'t happen.');
    return returnCallback(new Error('Camera open'), null);
  }

  // Ustaw flagę inicjalizacji i użycia kamery
  global.cameraInitiationInProgress = true;
  global.isCameraBeingUsed = true;
  console.log('[NEWLAND] Camera initiation and usage flags activated');

  const externallyClosedHandler = () => {
    // Zwalnianie flag w przypadku zewnętrznego zamknięcia
    global.isCameraBeingUsed = false;
    global.cameraInitiationInProgress = false;
    console.log('[NEWLAND] Camera closed externally, flags released');
  ...
  const cleanup = () => {
    ...
    // Zwalnianie flag przy czyszczeniu
    global.isCameraBeingUsed = false;
    global.cameraInitiationInProgress = false;
    console.log('[NEWLAND] Camera cleanup complete, flags released');
  }
  ...
  try {
    ...
    // Flaga inicjalizacji może być zwolniona po utworzeniu streama
    global.cameraInitiationInProgress = false;
    console.log('[NEWLAND] Camera stream created, initiation flag released');
    ...
    // Zwalnianie flag w przypadku błędu
    global.isCameraBeingUsed = false;
    global.cameraInitiationInProgress = false;
    console.log('[NEWLAND] Camera error occurred, flags released:', err.message);
    returnCallback(err, null);
  }
}
...
const delayedFacephoto = (callback) => {
  ...
  // Sprawdź czy niedawno naciśnięto Cancel
  const timeSinceLastCancel = Date.now() - global.lastCancelTime;
  if (global.lastCancelTime > 0 && timeSinceLastCancel < CANCEL_COOLDOWN) {
    console.log(`[NEWLAND] Too soon after cancel operation (${timeSinceLastCancel}ms). Minimum cancel cooldown: ${CANCEL_COOLDOWN}ms`);
    return callback(new Error('Cancel cooldown active'), null);
  }
  
  // Sprawdź czy była niedawna operacja kamery (w ciągu ostatnich 5 sekund)
  const timeSinceLastCameraOp = Date.now() - global.lastCameraOperation;
  if (global.lastCameraOperation > 0 && timeSinceLastCameraOp < CAMERA_OP_COOLDOWN) {
    console.log(`[NEWLAND] Too soon after last camera operation (${timeSinceLastCameraOp}ms). Minimum cooldown: ${CAMERA_OP_COOLDOWN}ms`);
    return callback(new Error('Camera operation cooldown active'), null);
  }
  
  // Sprawdź czy globalny lock jest aktywny
  if (global.uiActionLock) {
    console.log('[NEWLAND] UI action lock active, ignoring facephoto request');
    return callback(new Error('User interface locked'), null);
  }
  
  // Sprawdź czy kamera jest już otwarta
  if (isOpened()) {
    console.log('[NEWLAND] Camera is already open. Shouldn\'t happen.');
    return callback(new Error('Camera open'), null);
  }
  
  // Zapisz czas operacji kamery
  global.lastCameraOperation = Date.now();
  
  // Aktywuj globalny lock UI
  global.uiActionLock = true;
  console.log('[NEWLAND] UI action lock activated for facephoto');
  
  // Ustaw automatyczne odblokowanie po określonym czasie jako zabezpieczenie
  const autoUnlockTimeout = setTimeout(() => {
    if (global.uiActionLock) {
      global.uiActionLock = false;
      console.log('[NEWLAND] UI action lock auto-released after timeout');
    }
  }, 10000); // 10 sekund jako zabezpieczenie awaryjne
  ...
  delayedPhoto(device, { mode }, (err, result) => {
    // Zwolnij globalny lock po zakończeniu operacji
    clearTimeout(autoUnlockTimeout);
    setTimeout(() => {
      global.uiActionLock = false;
      console.log('[NEWLAND] UI action lock released after facephoto');
    }, 1000); // Zwiększony czas blokady do 1 sekundy
...
function scanPhotoCard(callback) {
  ...
  // Sprawdź czy globalny lock jest aktywny
  if (global.uiActionLock) {
    console.log('[NEWLAND] UI action lock active, ignoring photo card scan request');
    return callback(new Error('User interface locked'), null);
  }
  ...
  // Aktywuj globalny lock UI
  global.uiActionLock = true;
  console.log('[NEWLAND] UI action lock activated for photo card scan');
  ...
  capture(device, { mode }, (err, result) => {
    // Zwolnij globalny lock po zakończeniu operacji
    clearTimeout(autoUnlockTimeout);
    setTimeout(() => {
      global.uiActionLock = false;
      console.log('[NEWLAND] UI action lock released after photo card scan');
    }, 1000); // Zwiększony czas blokady do 1 sekundy
    
    if (err) {
      console.log(`[NEWLAND] Error in photo card scan: ${err.message}`);
      return callback(err, null);
    }
    
    callback(null, result);
  }, _.noop, async ({ frame, width, height }) => {
    try {
