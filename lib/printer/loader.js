...
const sanei = () => require('./sanei-sk4')

const ZEBRA_MODEL = 'Zebra-KR-403'
const NIPPON_MODEL = 'Nippon-2511D-2'
const GENMEGA = 'genmega'
const SANEI_MODEL = 'Sanei-SK4-21'
...
function configureAsSaneiWithCups (printerCfg) {
  const address = printerCfg.address
  ...
  fs.writeFile(deviceConfigPath, jsonDeviceConfig, err => {
    if (err) {
      console.log('PRINTER: failed updating device_config', err)
    } else {
      console.log('PRINTER: successfully updated device_config with CUPS configuration')
    }
  })
}
...
function load () {
  return new Promise((resolve, reject) => {
    ...
    // Check for Sanei SK4-21
    if (model === SANEI_MODEL || printerCfg.address === 'SANEI-SK4-21-Series') {
      console.log('[PRINTER] Detected Sanei SK4-21 printer, using CUPS mode')
      return resolve(sanei())
    }

    // Check for genmega here, we should force the model field in the config
    if (model === GENMEGA) return resolve(genmega())
