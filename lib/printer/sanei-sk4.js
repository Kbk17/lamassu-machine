...
const printReceipt = (data, printerCfg, receiptConfig) => {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (data.session) {
        cmd.push('ID Transakcji:\n')
        // Split long session ID into multiple lines (max 32 chars per line)
        const sessionId = data.session
        const maxLineLength = 32
        for (let i = 0; i < sessionId.length; i += maxLineLength) {
          cmd.push('  ' + sessionId.substring(i, i + maxLineLength) + '\n')
        }
      } else if (data.txId) {
        cmd.push('ID Transakcji:\n')
        // Split long transaction ID into multiple lines
        const txId = data.txId
        const maxLineLength = 32
        for (let i = 0; i < txId.length; i += maxLineLength) {
          cmd.push('  ' + txId.substring(i, i + maxLineLength) + '\n')
        }
      }
      ...
      // Print address - ALWAYS display if available
      if (data.address) {
        cmd.push('\n')
        cmd.push(alignCenter)
        cmd.push(emphasizedOn)
        ...
        // Format address with spaces every 4 characters for better readability
    ...
    try {
      ...
      if (data.batch) {
        ...
        if (data.batch.id && data.batch.operationType) {
          ...
          cmd.push('ID Partii:\n')
          // Split long batch ID into multiple lines
          const batchId = data.batch.id
          const maxLineLength = 32
          for (let i = 0; i < batchId.length; i += maxLineLength) {
            cmd.push('  ' + batchId.substring(i, i + maxLineLength) + '\n')
          }
        } else {
        ...
        if (data.batch.deviceId) {
          cmd.push('ID Maszyny:\n')
          // Split long device ID into multiple lines
          const deviceId = data.batch.deviceId
          const maxLineLength = 32
          for (let i = 0; i < deviceId.length; i += maxLineLength) {
            cmd.push('  ' + deviceId.substring(i, i + maxLineLength) + '\n')
          }
        }
