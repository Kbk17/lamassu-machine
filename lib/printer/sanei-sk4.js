...
const printQRCode = (cmd, str) => {
  ...
  try {
    ...
    // 2. Ustawienie rozmiaru kodu QR (zwiększenie z 4 na 8 dla uzyskania wymiarów 4cm x 4cm)
    cmd.push(Buffer.from([0x1d, 0x28, 0x6b, 0x03, 0x00, 0x31, 0x43, 0x08]))
...
// Zamiast funkcji printQRCode, zastąpimy ją nową funkcją printAddressText
const printAddressText = (cmd, str) => {
  // Sprawdź, czy str istnieje i jest stringiem
  if (!str || typeof str !== 'string') {
    console.error('[PRINTER] Error: Invalid address data:', str)
    return
  }
  
  try {
    // Zamiast kodu QR, wydrukuj czytelny tekst w ramce
    cmd.push(alignCenter)
    cmd.push(emphasizedOn)
    cmd.push('--- ADDRESS DATA ---\n')
    cmd.push(emphasizedOff)
    ...
    console.error('[PRINTER] Error printing address text:', err.message)
  }
}
...
const printReceipt = (data, printerCfg, receiptConfig) => {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Drukowanie adresu - tylko raz, bez kodu QR
      if (data.address) {
        cmd.push('\n')
        cmd.push(alignCenter)
        cmd.push(emphasizedOn)
...
const printWallet = (wallet, printerCfg, code) => {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      printAddressText(cmd, wallet.privateKey)
