...
const { gql, Client, fetchExchange } = require('@urql/core')
...
const fetch = (uri, options) => {
  ...
  const fetchOptions = _.flow(
    _.update('headers', _.set('date', new Date().toISOString())),
    _.set('signal', controller.signal)
  )(options)
  
  return node_fetch(uri, fetchOptions)
